...
% ---------- Title ----------
\Title{Symmetry-Aware Attention Networks for Student Engagement Detection: A Lightweight Temporal Approach}
...
\keyword{symmetry-aware attention; student engagement detection; facial expression recognition; lightweight CNN; temporal modeling; BiLSTM; edge computing; class imbalance; video analysis}
...
\\nFrom a symmetry perspective, student engagement detection can be understood as identifying symmetry-breaking phenomena in facial behavior~\cite{fredricks2004school}. Engaged states exhibit symmetric facial muscle activation patterns, while disengaged states (boredom, frustration, confusion) manifest as asymmetric deviations from this baseline~\cite{kleinsmith2013affective}. This symmetry-breaking principle motivates the design of attention mechanisms that can capture both symmetric (engaged) and asymmetric (disengaged) facial patterns at multiple spatial scales.
...
\begin{enumerate}
\item We propose a symmetry-aware attention framework with a multi-scale Channel-Spatial Attention Module (CSAM), which employs parallel 3$\times$3, 5$\times$5, and 7$\times$7 convolutions with adaptive fusion weights to capture facial features at multiple spatial scales. CSAM is designed to detect both symmetric (engaged) and asymmetric (disengaged) facial patterns through joint channel and spatial recalibration. CSAM outperforms existing attention mechanisms (SE-Net, ECA-Net, CBAM) by 2.4--3.7\% on the same backbone, with only 0.3\% parameter overhead.
