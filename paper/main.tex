...
The model achieves 74.5\% validation accuracy and 62.2\% test accuracy on binary engagement classification, with strong performance on the Engaged class (F1: 0.84) but weaker performance on the Disengaged class (F1: 0.33). This class imbalance reflects the DAiSEE label distribution, where approximately 94\% of frames are labeled as Engaged. Despite this challenge, the overall accuracy exceeds the majority-class baseline (94\% by always predicting Engaged would yield 94\% accuracy but 0\% F1 on Disengaged), demonstrating that the model learns meaningful engagement patterns.

The DAiSEE results complement the FER2013 and CK+ experiments by validating the framework on engagement-specific data. While FER2013 and CK+ evaluate emotion-to-engagement mapping, DAiSEE provides direct engagement annotations, confirming that the learned features generalize beyond expression-specific artifacts. The 62.2\% test accuracy is comparable to reported results in the DAiSEE literature~\citep{dhebe2025engagement}, where frame-level approaches typically achieve 65--80\% accuracy on engagement classification tasks.
...
On FER2013, MobileNetV3+CSAM achieves 70.5\% accuracy with 1.26M parameters and 0.068G FLOPs, surpassing ResNet-50 (64.0\%, 25.6M) by 3.8\% while requiring 2.6$\times$ less energy per inference. EfficientNet-B0+CSAM reaches 69.1\% with 5.4M parameters. The CSAM module contributes 1.3--5.0\% accuracy gain with negligible overhead, and the focal weighted loss with weighted sampling improves minority-class F1 by 1.5--2.4\% over standard approaches. Cross-dataset evaluation on CK+ (71.9\% accuracy without fine-tuning, 114 test samples) provides preliminary evidence of generalizability. Validation on DAiSEE---an engagement-specific dataset with direct annotations---achieves 74.5\% validation accuracy and 62.2\% test accuracy on binary engagement classification, confirming that the framework generalizes beyond emotion-to-engagement mapping.
