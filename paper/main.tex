...
% ---------- Title ----------
\Title{A Real-Time Visual Sensing System for Classroom Engagement Monitoring Using Multi-Scale Temporal Neural Networks}
...
% ---------- Abstract (English) ----------
\abstract{%
This paper presents a real-time visual sensing system for automated classroom engagement monitoring using camera-based facial expression analysis. The system employs a lightweight multi-scale neural network architecture that processes video streams from standard classroom cameras to detect student engagement levels without requiring specialized hardware or wearable sensors. Our approach combines a novel multi-scale attention module with temporal sequence modeling to capture both spatial and temporal patterns in facial expressions indicative of engagement states. The proposed multi-scale attention mechanism employs parallel convolutions at three spatial scales (3×3, 5×5, 7×7) with adaptive fusion, enabling the system to detect engagement cues across different facial regions and expression intensities. For temporal analysis, we integrate a bidirectional LSTM module with attention-weighted pooling to aggregate frame-level predictions into video-level engagement classifications, addressing the inherent temporal dynamics of student attention. The system is evaluated on three benchmark datasets: FER2013 (70.5% accuracy, 5-fold cross-validation), CK+ (71.9% cross-dataset accuracy), and DAiSEE (94.8% video-level accuracy). The complete pipeline achieves real-time processing at 53 frames per second on standard GPU hardware with only 1.26M parameters, demonstrating feasibility for deployment in actual classroom environments. Ablation studies confirm the contributions of multi-scale attention (+2.4–3.7% over single-scale baselines), temporal aggregation (61.2% → 94.8% video-level accuracy), and class-balanced training (+1.5–2.4% F1 on minority classes). These results demonstrate that camera-based visual sensing systems can provide practical, non-intrusive engagement monitoring for educational environments.
}
 for frame sequence encoding and attention-weighted temporal pooling, which learns to emphasize engagement-relevant temporal segments.
...
\keyword{visual sensing system; classroom monitoring; engagement detection; facial expression recognition; multi-scale attention; temporal modeling; lightweight neural network; real-time processing; edge deployment}
...
\begin{enumerate}
\item We propose a multi-scale Channel-Spatial Attention Module (CSAM) for visual sensing applications, which employs parallel 3$\times$3, 5$\times$5, and 7$\times$7 convolutions with adaptive fusion weights to capture facial features at multiple spatial scales. CSAM is designed for lightweight architectures used in real-time sensing systems. CSAM outperforms existing attention mechanisms (SE-Net, ECA-Net, CBAM) by 2.4--3.7\% on the same backbone, with only 0.3\% parameter overhead.
