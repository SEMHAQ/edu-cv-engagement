...
The key novelty lies in the \textit{combination} of these four contributions into a coherent framework. While individual components (attention modules, focal loss, transfer learning) are established techniques, their integration for engagement detection---with CSAM's mobile-optimized design, focal weighted loss for joint class balancing and hard-example mining, emotion-to-engagement mapping with theoretical grounding, and deployment-oriented evaluation across three datasets---has not been previously demonstrated.
